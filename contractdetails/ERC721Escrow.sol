...
contract ERC721 {
    ...
    function bidNFT(uint256 tokenID) internal {
        require(
            block.timestamp < auctionEndTime[tokenID],
        ...
        require(
            ...
            "Bidding value is zero, can not be entered!"
        );
    ...
    function getAuctionPeriod(uint256 tokenID)
        public
        view
        returns (uint256, uint256)
    {
    ...
    function isPriorBidPresent(uint256 _tokenID, address _bidder)
        public
        view
        returns (bool)
    {
        ...
        for (uint256 i = 0; i < length; i++) {
            if (
                tokenWiseBidParticipatorList[_tokenID][i]
    ...
    function bidding(
        ...
        uint256 _tokenID,
        string memory bidParticipatorName
    ...
    function placeFirstBid(
        ...
        require(
            isPriorBidPresent(_tokenID, _bidder) == false,
            "Since you have already bid and entered some funds as a BID AMOUNT into our platform, please collect your funds first and re-bid here."
        );
    ...
    function updatePreviousBidAmountToZero(
        address payable _bidder,
        uint256 _tokenID
    ) internal returns (bool) {
        require(isPriorBidPresent(_tokenID, _bidder) == true);
    ...
    function reBid(
        ...
        require(
            isPriorBidPresent(_tokenID, _bidder) == true,
            "Place your first bid then have a try to re-bidding."
        );
    ...
    function takeBidWinnerName(uint256 tokenID, string memory bidderName)
        internal
        returns (string memory)
    {
        return highestBidderName[tokenID] = bidderName;
    }

    function declareWinner(uint256 tokenID, string calldata bidWinnerName)
        external
        returns (
            address,
            uint256,
        ...
        )
    {
        if (block.timestamp < auctionEndTime[tokenID]) {
            ...
            require(
                ...
                "Sorry, You are not the owner of this NFT, You can not close this bid."
            );

            bidEnded[tokenID] = true;

            uint256 nftBiddingTime = nftListByNFTOwner[msg.sender][tokenID]
                .nftMintTime;

            address payable nftBidder = nftListByNFTOwner[msg.sender][tokenID]
                .tokenOwner;

            string memory creatorName = nftListByNFTOwner[nftOwner][tokenID]
                .createrName;
            ...
            nftDetails = NFTDetails(
                creatorName,
                takeBidWinnerName(tokenID, bidWinnerName),
                nftOwner,
                tokenID,
                nftBiddingTime,
                price,
                nftListByNFTOwner[nftOwner][tokenID].nftName,
                true,
                nftListByNFTOwner[nftOwner][tokenID].uri,
                nftListByNFTOwner[nftOwner][tokenID].description
            );
            ...
            for (
                uint256 i = 0;
                i < tokenWiseBidParticipatorList[tokenID].length;
                i++
            ) {
                bidderAddress = tokenWiseBidParticipatorList[tokenID][i]
            ...
            return (
                nftListByNFTOwner[highestBidder[tokenID]][tokenID].tokenOwner,
                tokenID,
                true
            );
        }
        ...
        )
    {
        return (
            ...
            NFTInfo[tokenID].isNFTBiddingDone
        );
    }

    function withdrawFunds(uint256 _tokenID, address _bidder)
        external
        returns (bool)
    {
        _bidder = msg.sender;
    ...
    function getBiddersList(uint256 tokenID)
        public
        view
        returns (Participants[] memory l)
    {
