...
library Bid {
    ...
    function declareWinner(uint256 tokenID, Data storage libdata) external {
        if (block.timestamp < libdata.auctionEndTime[msg.sender][tokenID]) {
            ...
            uint256 length = libdata.tokenWiseBidParticipatorList[msg.sender][tokenID].length;
...
contract ERC1155Matellio is ERC1155 {
        ...
        returns (address[] memory)
    {
        ...
        for(uint256 i=0; i<length; i++){
            if(libdata.totalTokensOwnedByOwner[tokenID][libdata.tokenWiseAllOwners[tokenID][i]] == 0){
    ...
    function allNFTOwners(uint256 tokenID) external view returns(address[] memory){

        return libdata.tokenWiseAllOwners[tokenID];

    }

    function totalTokensOwnedByOwner(uint256 tokenID, address nftOwner) external view returns(uint256){
    ...
    function setAuctionPeriod(uint256 _biddingTime, uint256 tokenID, uint256 totalTokensInBidding)
        external
        returns (uint256, uint256)
    {
        ...
        libdata.totalBidTokensAmount[msg.sender][tokenID][libdata.auctionEndTime[msg.sender][tokenID]] = totalTokensInBidding;
    ...
    function totalBidTokens(uint256 tokenID, address nftOwner) public view returns(uint256){
        return libdata.totalBidTokensAmount[nftOwner][tokenID][libdata.auctionEndTime[nftOwner][tokenID]];
    }

    function DeclareBIDWinner(uint256 tokenID) public {
        address tokenAndOwnerwiseHighestBidder = libdata.highestBidder[msg.sender][tokenID];
        uint256 bidtokenamount = libdata.totalBidTokensAmount[msg.sender][tokenID][libdata.auctionEndTime[msg.sender][tokenID]];
        safeTransferFrom(
            msg.sender,
        ...
        uint256 totalTokensOwnedByAuctioneer = libdata.totalTokensOwnedByOwner[tokenID][msg.sender];

        totalTokensOwnedByAuctioneer = totalTokensOwnedByAuctioneer - bidtokenamount;

        libdata.totalTokensOwnedByOwner[tokenID][tokenAndOwnerwiseHighestBidder] = libdata.totalTokensOwnedByOwner[tokenID][tokenAndOwnerwiseHighestBidder] + bidtokenamount;

        libdata.highestBidder[msg.sender][tokenID] = 0x0000000000000000000000000000000000000000;
    ...
    function getBiddersList(uint256 tokenID, address nftOwner) external view returns(Participants[] memory p){
